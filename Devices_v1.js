...
function setNumberOfTasks() {
	...
	var lDevices = lib();
	var arrDevices = lDevices.entries();
	var lTasks = libByName(LIB_TASKS);
	//log("\ngetNumberOfTasks: " + lDevices.name + " " + arrDevices.length);
	//log("\n  " + lTasks.name);
	//Цикл по подразделениям
	for (var j=0; j<arrDevices.length; j++) {
		var device = arrDevices[j];
		//log("\n  подразделение: " + device.name);
		var arrAllTasks = lTasks.linksTo(device);
		...
		if (result == 0) {device.set(TASKS_NUMBER, "");} else {device.set(TASKS_NUMBER, ICO_TASKS + result);}
	}
