...
function setNumberOfTasks() {
	...
	var lPartners = lib();
	var arrPartners = lPartners.entries();
	var lTasks = libByName(LIB_TASKS);
	//log("\ngetNumberOfTasks: " + lPartners.name + " " + arrPartners.length);
	//log("\n  " + lTasks.name);
	//Цикл по подразделениям
	for (var j=0; j<arrPartners.length; j++) {
		var partner = arrPartners[j];
		//log("\n  подразделение: " + partner.name);
		var arrAllTasks = lTasks.linksTo(partner);
		...
		if (result == 0) {partner.set(TASKS_NUMBER, "");} else {partner.set(TASKS_NUMBER, ICO_TASKS + result);}
	}
