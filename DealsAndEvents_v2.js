...
var DealsAndEvents = (function{
	...
	//------------------------------------------------------
	function pCheckDeal(incomeDeal) {

		log("\n-> pCheckDeal(incomeDeal)");
	...
	//----------------------------------------------------------
	function pShiftDate(bForward, incomeDeal) {

		log("\n-> pShiftDate(bForward, incomeDeal)");
	...
	//----------------------------------------------------------
	function pGetDealName() {
		//Обрабатываемое дело
		var deal = entry();
		//Поля
		var FCategory = deal.field (CATEGORY);
		var FName = deal.field (NAME);
		return pGetIcon(FCategory) + " " + pGetName(FName);
	}

	//----------------------------------------------------------
	//Функция для вывода типа
	//----------------------------------------------------------
	function pGetDealType() {
		//Обрабатываемое дело
		var deal = entry();
		//Поля
		var FType = deal.field (TYPE);
	...
	//----------------------------------------------------------
	function pGetDealStatus() {
		//Обрабатываемое дело
		var deal = entry();
		//Поля
		var FStatus = deal.field (STATUS);
		var ico = pGetIcon(FStatus);
		if (ico==ICO_PLAN) {
			return "";
		} else {
			return ico;
	...
	//======================================================
	return {
		checkDeal: pCheckDeal,
		shiftDate: pShiftDate,
		shiftAuto: pShiftAuto,
		daysLeft: pDaysLeft,
		getDealName: pGetDealName,
		getDealType: pGetDealType,
		getDealStatus: pGetDealStatus;
	};
})();
